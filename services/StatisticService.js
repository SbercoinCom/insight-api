...
StatisticService.prototype.start = function (callback) {
    ...
    return async.waterfall([function (callback) {
        ...
        self.node.services.sbercoind.on('tip', self._rapidProtectedUpdateTip.bind(self));
...
StatisticService.prototype.updateOrCreateDay = function (date, data, next) {
    ...
    return async.waterfall([function (callback) {
        ...
        if (subsidy) {
            ...
            dayBN.supply.sum = SupplyHelper.getTotalSupplyByHeight(block.height).mul(1e7);
...
StatisticService.prototype.getSupply = function (days, next) {
    ...
    return self.getStats(days, function (err, stats) {
        ...
        stats.forEach(function (day) {
            ...
            results.push({
                date: self.formatTimestamp(day.date),
                sum: sumBN.gt(0) ? sumBN.dividedBy(1e7).toString(10) : '0'
            });
...
StatisticService.prototype.getTotalSupply  = function() {
    var blockHeight = this.node.services.sbercoind.height;

    var supply = (new BigNumber(183949683*5000)).plus((blockHeight - 5000) * 300);
