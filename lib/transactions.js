...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.sbercoind.height - transaction.height + 1;
  }
  ...
  transformed.valueOut = transaction.outputSatoshis / 1e7;
  transformed.size = transaction.hex.length / 2; // in bytes
  if (!transaction.coinbase) {
    transformed.valueIn = transaction.inputSatoshis / 1e7;
    transformed.fees = transaction.feeSatoshis / 1e7;
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.value = input.satoshis / 1e7;
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  var transformed = {
    value: (output.satoshis / 1e7).toFixed(8),
    n: index,
    scriptPubKey: {
      hex: output.script
    }
...
TxController.prototype.transformSbercoinTransaction = function(transaction) {
