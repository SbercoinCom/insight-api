...
BlockController.prototype.getBlockByHash = function (hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.sbercoind.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.sbercoind.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype.blockIndex = function (req, res) {
    ...
    this.node.services.sbercoind.getBlockHeader(parseInt(height), function (err, info) {
        if (err) {
            return self.common.handleErrors(err, res);
        }
        res.jsonp({
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
    ...
    if (summaryCache) {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.sbercoind.client.getBlock(hash, function (err, response) {
            ...
            var confirmations = self.node.services.sbercoind.height - block.height + 1;
...
BlockController.prototype.list = function (req, res) {
    ...
    return self.node.services.sbercoind.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
